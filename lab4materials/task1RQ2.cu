...
__global__ void
op_cuda_grayscale_rq2(uint32_t *dst, int rowstride,
                      int width, int height, int pxpPerThread)
{
  ...
  // Iterate over tiles assigned to this thread in the Y direction
  for (int yy = y; yy < height; yy += strideY) {
    // Process the tiles in the X direction
    for (int xx = x; xx < width; xx += strideX) {
      // Iterate within the tile this thread is responsible for
      for (int tile_y = 0; tile_y < pxpPerThread; tile_y++) {
        for (int tile_x = 0; tile_x < pxpPerThread; tile_x++) {
            // Compute the coordinates of the current pixel
            int pixel_x = xx + tile_x;
            int pixel_y = yy + tile_y;

            // Boundary check to ensure we don't access out-of-bounds pixels
            if (pixel_x >= width || pixel_y >= height)
                continue;

            // Load the pixel
            rgba_t color;
            RGBA_unpack(color, *image_get_pixel_data(dst, rowstride, pixel_x, pixel_y));

            // Compute the grayscale intensity
            float intensity = cuda_compute_intensity(color);

            // Create the grayscale pixel: R, G, B = intensity, A = 1.0
            rgba_t gray;
            RGBA(gray, intensity, intensity, intensity, 1.0f);

            // Store it back
            RGBA_pack(*image_get_pixel_data(dst, rowstride, pixel_x, pixel_y), gray);
        }
      }
    }
  }
...
run_cuda_kernel(image_t *background)
{
   ...
   */
  op_cuda_grayscale_rq2<<<grid_dim, block_dim>>>(d_data, background->rowstride, background->width, background->height, 1);
